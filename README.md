...
`ASIA` 로 시작하는 임시 자격증명은 세션 토큰 없이는 살아있어도 거부당한다.
근처에서 Secret Access Key와 세션 토큰을 모두 찾은 경우(예: `.env` 파일에 세 값이
함께 유출된 경우)에만 `X-Amz-Security-Token` 헤더까지 서명에 포함해 검증을
시도하고, 세션 토큰을 못 찾으면 지금처럼 검증을 건너뛴다.
