...
secrethound는 현재 파일과 **전체 커밋 히스토리**를 함께 검사해서 이런 키를 찾아낸다.
...
리포트를 열면 그 한 건이 이렇게 나온다.

| 심각도 | 위치 | 종류 | 값 |
|---|---|---|---|
| CRITICAL | `config.js@be880d7:1` | github-pat | `ghp_9m******2pXk` |

지금 파일만 보면 깨끗하다. 이미 환경변수로 옮겼기 때문이다. 그런데 `@be880d7`,
즉 **과거 커밋에는 토큰이 그대로 남아있다.** 이런 걸 찾는 도구다.
...
2. 압축을 푼다
3. **`secrethound.exe` 를 두 번 클릭**하고, 창에서 검사할 폴더를 고른다

끝이다. 검사가 끝나면 결과가 브라우저에 자동으로 열린다. 설치 과정이 없고 명령어를
칠 일도 없다. 지울 때는 폴더째 삭제하면 된다.
...
폴더를 **exe 위로 끌어다 놓아도** 검사가 시작된다. 여러 개를 한꺼번에 놓아도 된다.

프로젝트를 모아두는 폴더 하나만 골라도 된다. 그 아래에 있는 git 저장소를 찾아
각각 검사한다. 저장소가 한 겹 더 감싸여 있어도(`projects/myapp/myapp`) 찾아내며,
디스크 전체를 훑지 않도록 3단계까지만 내려간다.
...
리포트는 exe 옆의 `secrethound-결과/` 폴더에 두 형식으로 저장된다.

| 형식 | 쓰임 |
|---|---|
| `.html` | 브라우저에서 표로 본다. 심각도 필터·검색·정렬이 된다 (검사 후 자동으로 열린다) |
| `.md` | 메모장·노션·깃허브에 그대로 붙여넣는다 |

파일 이름에는 **검사한 폴더 이름**이 붙어(`secrethound-report-alpha.html`) 어느
프로젝트의 결과인지 구분되고, 같은 폴더를 다시 검사하면 `-1`, `-2` 가 붙어 이전
결과를 덮어쓰지 않는다.

리포트에는 심각도(`CRITICAL`/`HIGH`/...), 위치(`파일:줄번호`), 키 종류, 값의 일부
(마스킹됨)가 나온다. 아무것도 안 나오면 정상이다.

### 키가 나왔다면

**파일에서 지우기 전에 발급처에서 키를 먼저 폐기(revoke)해야 한다.** 파일에서 지우고
커밋해도 키 자체는 여전히 살아있고, 이미 누군가 저장소를 복사해 갔을 수도 있다.
폐기가 먼저이고 파일 정리는 그 다음이다.

어디서 어떻게 폐기하는지는 리포트의 **"대응 방법"** 절에 키 종류별로 적혀 있다.
히스토리에서 발견된 경우 `git filter-repo` 명령도 함께 나온다.

## 터미널에서 쓰기 (선택)

여기서부터는 CLI로 쓰거나 CI에 붙일 때만 필요한 내용이다. 위의 더블클릭 사용법만으로
충분하다면 읽지 않아도 된다.

같은 실행 파일이 터미널에서는 평소처럼 CLI로 동작한다. 더블클릭으로 실행됐을 때만
폴더 선택 창을 띄우는데, 콘솔에 이 프로세스만 붙어 있는지로 그 둘을 구분한다.

### 받는 방법

릴리스 zip을 받아 압축만 풀어도 되고(Go 불필요), Go가 있으면 직접 설치해도 된다.
...
`check` 는 더블클릭했을 때와 같은 일을 한다 — 플래그 없이 항상 같은 기본값으로
검사하고 리포트를 만들어 브라우저로 연다. `scan` 은 플래그로 세부를 제어하고 종료
코드로 탐지 여부를 알리는 CI용이다. 시크릿을 찾아도 `check` 는 0으로 끝나고
`scan` 은 1로 끝난다.
...
# 저장소를 모아둔 폴더를 지정하면 그 아래 저장소를 전부 검사
secrethound check ~/projects

# 폴더를 여러 개 직접 나열해도 된다
secrethound check ./repo-a ./repo-b ./repo-c
...
여러 폴더를 검사하면 저장소마다 리포트가 따로 만들어지고, 브라우저 탭이 그만큼
열리지 않도록 리포트가 모인 폴더를 대신 열어준다.
...
한 번만 한다. 현재 37개 룰 중 **20개**가 검증 대상이며 발급처는 17곳이다
(`secrethound rules` 로 확인 가능). 국내 서비스는 카카오(REST API 키·Admin 키)와
토스페이먼츠 시크릿 키를 검증한다. Google API Key·공공데이터포털 키·Twilio·Shopify·
네이버·NCP·포트원·JWT·PEM 개인키·DB 접속 문자열 등은 공통 엔드포인트가 없거나,
짝이 되는 값이 필요하거나, 발급처가 정해져 있지 않아 검증하지 않는다.
...
형식만 맞는 가짜 키를 17곳에 보내 이 표식이 여전히 유효한지 확인한다
(진짜 자격증명 불필요). 매주 월요일 CI([selfcheck.yml](.github/workflows/selfcheck.yml))로
자동 실행된다.

> selfcheck는 가짜 키가 항상 거부되는 **거부 경로**만 검증한다. 살아있는 키가
> `유효`로 판정되는 **성공 경로**는 7곳에서 확인했다. GitHub·GitLab·Slack·Discord·
> Telegram·npm은 진짜 키로, 토스페이먼츠는 공식 문서의 공개 테스트 키로 확인했다.
> 카카오 2곳은 아직 미확인이다. 사업자 인증·유료 계정이 필요한 나머지 8곳(AWS·Stripe·
> OpenAI 등)은 거부 경로만 확인된 상태다.
...
레이블된 코퍼스(실제 시크릿 49건 + 오탐 유발 케이스 34건)로 정확도를 측정한다.
...
| gitleaks v8.30.1 | 0.867 | 0.796 | 0.830 |

측정 과정에서 오탐 4건, 미탐 5건, 심각도 오분류 1건, 도달 불가능한 룰 1개,
그리고 **정답 레이블 자체의 오류 1건**을 찾아 고쳤다.

> 이 표는 "더 낫다"가 아니라 참고용이다 — 코퍼스를 직접 만들고 그에 맞춰 룰을
> 고쳤으므로 과적합이다. gitleaks와의 격차도 대부분 코퍼스에 국내 서비스 케이스가
> 많아서 생긴 것이다. 자세한 내용은 [eval/README.md](eval/README.md) 참고.

과적합을 확인하려고 실제 오픈소스 레포 5개(cobra·logrus·express·gin·flask, 16,388
커밋)도 스캔했다. 오탐은 4건이었고 원인은 2가지(Flask 문서의 예시 키, gin의 테스트
픽스처 개인키)뿐이었다. 전체 목표와 실측값, 재현 명령은 [평가지표.md](docs/평가지표.md)에
정리해 두었다.
